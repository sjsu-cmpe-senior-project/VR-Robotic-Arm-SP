...
public class InMoovArm extends Service implements IKJointAngleListener {
  ...
  transient public Adafruit16CServoDriver servoDriver;
  ...
  public InMoovArm(String n) throws Exception {
    ...
    //servoDriver = (Adafruit16CServoDriver) createPeer("servoDriver");
  ...
  public boolean connect(String port) throws Exception {
    ...
    // }
    if (servoDriver == null) {
      error("ServoDriver is invalid");
      return false;
    }

    // arduino.connect(port);
    servoDriver.attach("raspi", "1", "0x40");

    // if (!servoDriver.isConnected()) {
    //   error("arduino %s not connected", arduino.getName());
    //   return false;
    // }
    bicep.attach(servoDriver, 5, bicep.getRest(), bicep.getVelocity());
    rotate.attach(servoDriver, 9, rotate.getRest(), rotate.getVelocity());
    shoulder.attach(servoDriver, 8, shoulder.getRest(), shoulder.getVelocity());
    omoplate.attach(servoDriver, 7, omoplate.getRest(), omoplate.getVelocity());
